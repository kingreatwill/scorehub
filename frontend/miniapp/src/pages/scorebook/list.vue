...
async function load(reset: boolean) {
  ...
  paging.value = !reset
  try {
    const res = await listMyScorebooks(pageSize, reset ? 0 : nextOffset.value)
    const incoming = (res.items || []).filter(isScorebook)
    if (reset) {
      items.value = sortScorebooks(incoming)
    } else {
      ...
      items.value = sortScorebooks(merged)
    }
