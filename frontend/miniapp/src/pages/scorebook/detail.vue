...
async function saveBrowserQRCode() {
  ...
  if (!isMpWeixin.value) {
    ...
    // #ifdef H5
    try {
      const code = String(scorebook.value?.inviteCode || '').trim() || 'invite'
      const link = document.createElement('a')
      link.href = browserQRSrc.value
      link.download = `scorehub-browser-join-${code}.png`
      link.rel = 'noopener'
...
async function saveInviteCodeQR() {
  ...
  if (!isMpWeixin.value) {
    ...
    try {
      ...
      link.download = `scorehub-invite-${code}.png`
      link.rel = 'noopener'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
